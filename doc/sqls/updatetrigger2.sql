CREATE   TRIGGER trg_InsertPairedRI 
ON cdERA.[cdm ].test_related_identifiers
				...
				BEGIN
			    	...
			    	IF EXISTS ( SELECT 0 from metadata_document where identifier = @RI_DOI )	
					   ...
					   BEGIN
						   ...
						   IF EXISTS (SELECT  0 from test_related_identifiers where md_id = @RI_md and related_identifier = @ori_DOI)
				    			BEGIN
					    			SELECT  @pair_ri_ID = ri_id from test_related_identifiers where md_id = @RI_md and related_identifier = @ori_DOI	
					    			SET @DEBUGNEW += '
					    			...
					    			IF EXISTS (SELECT 0 from test_related_identifiers where md_id = @RI_md and related_identifier = @ori_DOI and rt_id =  @pair_rt_ID)
					    				BEGIN
											SET @DEBUGMESSAGE += '
					    			...
					    			ELSE
					    				BEGIN
											...
											UPDATE test_related_identifiers SET md_id=@RI_md, related_identifier=@ori_DOI, it_id=1, rt_id=@pair_rt_ID WHERE ri_id=@pair_ri_ID
					    				END 
				    			END 
				    		ELSE
				    			BEGIN
				...
				BEGIN
					...
					IF EXISTS (SELECT 0 from metadata_document where identifier = @RI_DOI )
					    BEGIN
							...
							DELETE FROM test_related_identifiers WHERE ri_id =  @pair_ri_ID
					    END 
		    	...
		    	BEGIN
				    ...
				    IF EXISTS ( SELECT  0 from test_related_identifiers where md_id = @RI_md and related_identifier = @ori_DOI )	
				    	BEGIN
						    SELECT  @pair_ri_ID = ri_id from test_related_identifiers where md_id = @RI_md and related_identifier = @ori_DOI						    	
									...
									UPDATE test_related_identifiers
										SET md_id=  @RI_md , related_identifier=  @ori_DOI , it_id=0, rt_id=+ @pair_rt_ID 
				    ...
				    ELSE
				    	BEGIN
					    	...
					    	INSERT INTO test_related_identifiers
								(md_id, related_identifier, it_id, rt_id)
								VALUES(@RI_md, @ori_DOI, 1, @pair_rt_ID);
				    	END 
		    	END  
		    ELSE
				BEGIN					     	
